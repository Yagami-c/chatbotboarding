...
export function Onboarding({ onDone }: OnboardingProps) {
  ...
  if (step === 1) return (
    <div className="flex-1 flex flex-col items-center justify-center px-8 bg-white text-center">
      ...
      <button onClick={goNext}
        className="w-full max-w-xs py-3.5 rounded-full bg-[#07C160] text-white font-bold text-base border-0 cursor-pointer active:bg-[#06AE56] transition-all shadow-[0_4px_16px_rgba(7,193,96,0.4)]">
        下一步 →
      </button>
      ...
      <div className="flex gap-2 mt-10">
        {[1,2,3].map(i=><span key={i} className={`w-2 h-2 rounded-full ${i===1?"bg-[#07C160]":"bg-[#e2e8f0]"}`}/>)}
      </div>
    </div>
  );
  ...
  if (step === 2) return (
    <div className="flex-1 flex flex-col px-6 pt-12 pb-6 bg-white overflow-y-auto">
      ...
      <button onClick={()=>{
        ...
        className="w-full py-3.5 rounded-full bg-[#07C160] text-white font-bold text-base border-0 cursor-pointer active:bg-[#06AE56] transition-all">
        确认
      </button>
      ...
      <div className="flex gap-2 justify-center mt-6">
        {[1,2,3].map(i=><span key={i} className={`w-2 h-2 rounded-full ${i===2?"bg-[#07C160]":"bg-[#e2e8f0]"}`}/>)}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col items-center px-6 pt-12 pb-6 bg-white">
      <div className="text-5xl text-center mb-4">📋</div>
      <h2 className="text-xl font-bold text-[#1a202c] text-center mb-2">开始之前先分析？</h2>
      ...
      <div className="flex gap-3 w-full mb-4">
        <button onClick={()=>onDone(smartMode,"assessment")}
          className="flex-1 py-3.5 rounded-full bg-[#07C160] text-white font-bold text-sm border-0 cursor-pointer active:bg-[#06AE56] transition-all">
          📝 开始分析
        </button>
        <button onClick={()=>onDone(smartMode,"quick-training")}
          className="flex-1 py-3.5 rounded-full bg-[#ebf8f0] text-[#07C160] font-bold text-sm border border-[#07C160] cursor-pointer active:bg-[#d4f5e3] transition-all">
          🏃 开启设备
        </button>
      </div>
      ...
      <div className="flex gap-2 justify-center mt-6">
        {[1,2,3].map(i=><span key={i} className={`w-2 h-2 rounded-full ${i===3?"bg-[#07C160]":"bg-[#e2e8f0]"}`}/>)}
      </div>
    </div>
  );
}
